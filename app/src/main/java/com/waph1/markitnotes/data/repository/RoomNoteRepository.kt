...
class RoomNoteRepository(
    ...
    override suspend fun permaDeleteNote(id: String) = withContext(Dispatchers.IO) {
        val root = rootDir ?: return@withContext
        ...
        val deletedFile = root.findFile(".Deleted")?.findFile(folder)?.findFile(fileName)
        deletedFile?.delete()
        
        noteDao.deleteNoteByPath(id)
    }

    override suspend fun permaDeleteNotes(noteIds: List<String>) = withContext(Dispatchers.IO) {
        noteIds.forEach { permaDeleteNote(it) }
    }
    ...
    suspend fun refreshNotes() = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            try {
                var inboxFolder = root.findFile("Inbox")
                if (inboxFolder == null) {
                    inboxFolder = root.createDirectory("Inbox")
                }
                
                // Move loose files from root to Inbox
                if (inboxFolder != null) {
                    root.listFiles().forEach { file ->
                        if (file.isFile && (file.name?.endsWith(".md") == true || file.name?.endsWith(".txt") == true)) {
                            try {
                                val content = readText(file)
                                val newFile = inboxFolder.createFile("text/markdown", file.name!!)
                                newFile?.let { nf ->
                                    context.contentResolver.openOutputStream(nf.uri)?.use { os ->
                                        OutputStreamWriter(os).use { it.write(content) }
                                    }
                                ...
                                android.util.Log.e("RoomNoteRepository", "Failed to move loose file ${file.name} to Inbox", e)
                            }
                        }
                    }
                }
